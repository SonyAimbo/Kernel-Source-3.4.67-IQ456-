...
bool Speaker_Init(void)
{
   ...
   mt_set_gpio_mode(GPIO_SPEAKER_EN_PIN,GPIO_MODE_00);  		// mt_set_gpio_mode_(129, 0);
   mt_set_gpio_pull_enable(GPIO_SPEAKER_EN_PIN,GPIO_PULL_ENABLE); 	// mt_set_gpio_pull_enable_(129, 1);
   mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); 			// mt_set_gpio_dir_(129, 1);
   mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ZERO); 			// mt_set_gpio_out_(129, 0);
   PRINTK("-Speaker_Init Success");
   return true;
}
...
void Sound_Speaker_Turnon(int channel)
{
    ...
    if(gsk_on)
		return;
    mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // mt_set_gpio_dir_(129, 1);
    mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ONE); // mt_set_gpio_out_(129, 1);
    udelay(2);
    mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ZERO);// mt_set_gpio_out_(129, 1);
    udelay(2);
    mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ONE); // mt_set_gpio_out_(129, 1);
